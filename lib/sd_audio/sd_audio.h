...
// Playback of WAV files stored in sd_audio_directory. The decoded samples go
// through the same buffer as network-streamed audio, so the light effects
// react to them the same way.

void sd_audio_init();

// Lists the playable files. See list_audio_files().
